...
class NFeItemsListWidget extends StatefulWidget {
  ...
  const NFeItemsListWidget({super.key, required this.items, required this.onBackToInput});
...
class _NFeItemsListWidgetState extends State<NFeItemsListWidget> {
  ...
  double get _totalGeral {
    return _items.fold(0.0, (sum, item) => sum + item.valorUnitario * item.quantidade);
  }
      ...
      children: [
        ...
        Expanded(
          child: _items.isEmpty
              ? const Center(
                  child: Text('Nenhum item processado', style: TextStyle(fontSize: 16, color: Colors.grey)),
                )
              : ListView.builder(
